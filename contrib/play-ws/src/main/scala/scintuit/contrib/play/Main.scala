...
import scintuit.api
import scintuit.contrib.play.executor.PlayWsExecutor
...
object Main extends App {
  ...
  val addBMO: CustomerIO[LoginError \/ Vector[Account]] =
    addLogin(11761, Seq(Credentials("j_username", "drbild1"), Credentials("password", "8xumdmaJHs5_MtY82FUcvTHZJ"))) map (_ map (_.accounts))
...
//  }
  ...
  def z(account: AccountId) = transactor.trans(customer).apply(transactions(account, DateTime.now minusMonths 2, DateTime.now plusDays 1))
...
//  println(x.run)
//  println(y.run)
  println(z(bmoAccount).run map (t => s"${t.`type`.getOrElse("")} ${t.payee.getOrElse("")}") mkString "\n")
//  println(a.run)
//  b.run
//  c.run
//  println(d.run)
