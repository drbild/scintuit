...
import scintuit.util.request.RequestEncoder

object PlayRequestEncoder extends PlayRequestEncoder

trait PlayRequestEncoder extends RequestEncoder {
  ...
  override def encodeCredentials(credentials: Seq[Credentials]): String =
    encode(credentials)((__ \ "credentials" \ "credential").write)

  override def encodeAnswers(answers: Seq[ChallengeAnswer]): String =
    encode(answers)((__ \ "challengeResponses"  \ "response").write)

  override def encodeType(typ: AccountType): String = {
    val label = typ match {
